...
| **B-20** | Does the LITERAL periodically-rebalanced (monthly, or another single cadence fixed before running), fixed-50/50 BTC+ETH `kelly_regime_v4` portfolio — R-50's own original candidate, run through its continuous (non-restarting) engine, unmodified split, unmodified cadence discipline — survive its own pre-registered falsification test and a first, single holdout read? | SIZE, N≈3 | OPEN | The one form of this idea no session has holdout-tested: R-50 found it only as an inner-validation byproduct; R-51's conservative branch deliberately tested a never-rebalanced variant instead (REJECTED on holdout); R-51's novel branch computed it only as a reference point for its own inverse-vol candidate, never authorized by its pre-registration to read the holdout on the reference itself. Attempt this only with a fresh, self-contained pre-registration (falsification test and promotion decision rule chosen before running anything, per every prior round's discipline) — do not treat R-50's inner-validation number, or either R-51 branch's numbers, as already-earned evidence for it. Two considerations should inform, but not by themselves decide, that pre-registration: R-51's conservative branch found the drawdown-only component of this exact effect fails on the 2023+ holdout despite being non-artifactual and falsification-clean, and attributed roughly 71% of the untested variant's larger inner-validation Sharpe edge specifically to the rebalancing act — the same return-side mechanism, not yet independently holdout-tested. A session finding this backlog item unpersuasive, or wary of spending a third holdout consultation on one underlying idea within a single research program, should run `scripts/paper_trade.py` again instead (B-06), which costs nothing and needs no new idea. |
