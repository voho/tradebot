...
| R-33 | Funding-rate gate on `kelly_regime_v4` — stand down above an expanding top-decile trailing funding rate (backlog B-05) | 08-19 | `experiments/funding_gate.py` — `FundingGatedKellyV4`, 6 variants, inner-train 2020/inner-validation 2021-22 (both inside the committed funding file's 2020-2023 coverage), one frozen config evaluated on the only funding-covered holdout year (2023) | **Bit-identical to `kelly_regime_v4` over the entire 2023 holdout** — same fills, fees, funding paid, on both markets. The expanding top-decile threshold was set by 2021's funding blow-off and never re-crossed in 2022-2023 (multiplier minimum over 210k bars: exactly 1.0). Paired bootstrap returns exact [0.000, 0.000]. Skeptic-confirmed: no lookahead, no unit error, mechanism verified live pre-2022 (drops to 0.0 on 19,584 bars) and dormant after. | **NEGATIVE** — zero measured effect, not a near-miss |
| R-34 | Smooth own-drawdown feedback multiplier on `kelly_regime_v4`'s exposure (Busseti/Boyd risk-constrained Kelly 2016 and arXiv:2603.01298; Chekhlov/Uryasev/Zabarankin CDaR feedback control, arXiv:1710.01503) | 08-19 | `experiments/drawdown_feedback.py` — `DrawdownFeedbackKelly`, 6 variants (cap/power/floor), inner-train 2017-2020/inner-validation 2021-22, one frozen config (`dd_cap=0.50, power=2.0`) evaluated on the 2023+ holdout and ETH falsification | Cuts holdout futures drawdown **−3.05pp [−5.34, −0.40]** — the one interval of four (2 markets × {growth, DD}) that excludes zero, skeptic-reproduced across 5 bootstrap seeds — but an order of magnitude under the 10pp promotion bar, and never beats `buy_and_hold`. ETH falsification does not falsify (2-6pp DD cut, same direction, no exposure-matching step so not the R-31 trap). Mostly inert because v4's own 2023+ drawdown (28-33%) never approached the 0.50 cap. | **NEGATIVE** — real but too small, and untested outside a regime that never stressed it |

### R-33/R-34 — a same-day parallel round, B-05 and a novel drawdown-feedback mechanism

Both dispatched the same day against `kelly_regime_v4`, the table's #1 strategy, per the operator's own initiative (not a single backlog row): branch A ran backlog item **B-05** (funding gate, marked NEXT); branch B designed a **novel** SIZE-attacking mechanism from fresh literature, since B-05 alone did not cover the brief. Each branch owned disjoint files (`experiments/funding_gate.py`+runner vs. `experiments/drawdown_feedback.py`+runner), neither committed, both reported in full including their negative verdicts, and an independent skeptic re-ran the surviving numerical claims from both before either entered this ledger — per ROUTINE.md's parallel-work rules. Full pre-registrations, tables and lessons for each are below; both fed a single skeptic pass rather than two, since neither branch's primary report showed anything by itself worth withholding from scrutiny.

**R-33 in detail.** Mechanism: `FundingGatedKellyV4` reuses `kelly_regime_v3`'s vote/vol-sizer unchanged (confirmed byte-identical `prepare()` logic) and multiplies the sizing scale by a funding-derived factor — trailing mean funding over `funding_lookback_days`, compared against an *expanding* quantile of that trailing mean at `funding_quantile`, ffilled and shifted one bar for causality margin. Attacks COST; not a duplicate of R-14 (measured funding as a cost, never gated), R-15 (delta-neutral carry harvest, a different strategy) or R-16 (measured the signal, never wired it in) — the first actual implementation of B-05. Data constraint stated up front: the committed funding file covers 2020-01→2023-12-31 only, so against `OOS_START=2023-01-01` this idea's holdout is **one year**, not the project's usual 3.6.

Pre-registered (verbatim, before 2023 was read): frozen `funding_lookback_days=7.0, gate_style="hard", gate_floor=0.0, funding_quantile=0.90, funding_quantile_min_days=60.0`, rest at v4 defaults. Promote only if P1 (beats `buy_and_hold` on the 2023 holdout, funded futures) and P2 (>±0.2 Sharpe or ≥10pp DD vs v4) and P3 (falsification: still beats/ties v4 at the 0.40% fee tier) and P4 (inner-train neighbourhood is a plateau) all hold. **Named failure mode, before looking:** an expanding top-decile threshold set by an early extreme regime might never be re-crossed, making the gate a one-time historical filter rather than an ongoing one — exactly what happened.

Configs evaluated: **6** (`hard_n3/n7/n14`, `smooth_n7/n14`, `smooth_n14_f0.5`), 24 backtests across both inner splits and markets. Every variant cut inner-train (2020-2021) drawdown below v4's (24.4%→18.3-20.5%) at flat-to-better return — a real plateau on the split that discriminates — but inner-validation (2022) tied all 6 variants to plain v4 exactly, giving zero information to select on. Holdout (2023, funded futures): `buy_and_hold` $8,040 (DD 48.7%) vs `kelly_regime_v4` **and** `funding_gate` both $2,393 (DD 28.4%) — bit-identical fills, fees and funding paid on both markets. Paired bootstrap (mean_block=30, n_boot=2000, `total_log_return`/`annualized_sharpe`): exact **[0.000, 0.000]** on both stats, both markets — the same zero-width signature R-29's self-test uses to validate the pipeline, here because the strategies really are identical. Direct check: the multiplier's minimum over all 209,953 bars of 2022-2023 is 1.0; over 2020-2021 it drops to 0.0 on 19,584 bars — the mechanism is live, just dormant in the only window available to test it.

**P1 FAIL** (v4 itself loses to holding this year: $2,393 vs $8,040 funded-futures, $2,038 vs $2,556 spot). **P2 FAIL** (Δ=0.000 exactly). **P3 PASS, vacuously** (no extra turnover existed for the fee tier to punish). **P4** true on inner-train, meaningless on the holdout. Causality: by-hand two-opposite-tampers check passes for all 6 variants (max diff before cut = 0.0). **Skeptic: CONFIRMED** — traced the expanding-quantile computation line by line (settlement `i` uses only settlements ≤ `i`, no lookahead), verified `SETTLEMENTS_PER_DAY=3` against the raw file (every gap exactly 8h, 4,382/4,382), and independently reproduced both the holdout bit-identity and the min-multiplier claim.

**Verdict: NEGATIVE**, and not a coin flip — there is no measured effect to round in either direction on the only holdout this idea could be tested against. **Lesson:** a top-decile gate on an *expanding* quantile calibrates itself against the single most extreme regime it has seen and goes dormant once that regime passes — a one-time historical filter, not an ongoing one, exactly the failure mode named before any code ran. **Next step:** retry with a *bounded rolling-window* quantile (180 or 365 days) so the threshold can descend as well as rise — a different, untested mechanism needing its own fresh pre-registration, since this session's holdout read means the current design cannot be revisited without downgrading a new result to in-sample. B-02 (extend the funding series past 2023) remains the cheapest way to give this whole idea class a holdout worth reading.

**R-34 in detail.** Mechanism: multiply v4's target exposure by a smooth, continuous, monotone-decreasing function of the strategy's *own* current drawdown from its running peak equity — not price drawdown, not a market vol signal — `target[i] = v4.target[i] × clip(1 − (dd[i]/dd_cap)^power, floor, 1.0)`. Grounded in Busseti/Boyd's risk-constrained Kelly gambling line (2016; and Devanathan, Rueter, Boyd, Candès, Hastie, Kochenderfer et al. 2026, "Single-Asset Adaptive Leveraged Volatility Control", arXiv:2603.01298) and the drawdown-modulated feedback control line (Chekhlov, Uryasev & Zabarankin's CDaR; arXiv:1710.01503) — a continuous response with **no hard floor and no ratchet**, deliberately unlike R-11's Grossman-Zhou cushion. Attacks SIZE, secondarily ERR. Not a duplicate of R-11 (a ratcheted wealth-floor cushion vs. a memoryless continuous response to current drawdown — and the measured DD cost here, 2-3pp, is nowhere near R-11's 28%-drawdown/return-destroyed failure) or R-28/R-31/R-32 (an e-process gate compared at matched *raised* exposure; this mechanism only ever throttles down, `multiplier ≤ 1` by construction, so R-31's matched-risk trap does not apply the same way — checked anyway via the ETH test below).

Causality mechanics: a second causal forward pass computes a notional-implied equity proxy from the strategy's own *gated* position one step in the past against each bar's own return, then `peak = max(peak, equity)` and `dd` using only values through the current step; the resulting position takes effect at the next bar's open, the same contract v4 itself uses.

Pre-registered: frozen `dd_cap=0.50, power=2.0, floor=0.0, gate_deadband=0.10` — 0.50 sits above v4's own realized drawdown in every inner-split cell, so the multiplier stays near 1.0 through v4's ordinary range and only bites approaching the tail. Promote only if P1 (beats `buy_and_hold` OOS), P2 (>±0.2 Sharpe or ≥10pp DD cut vs v4), P3 (falsification: DD-cut direction replicates on ETH, unrefit, without re-matching exposure — explicitly checked against the R-31 trap), P4 (plateau not peak) all hold. **Stated prediction before looking:** the multiplier only ever reduces exposure, so — matching R-28/R-31/R-32 against this bull holdout — P1 fails, and the open question is whether the DD cut clears 10pp and survives ETH.

Configs evaluated: **6** (`v1_linear_tight` … `v6_convex_floor`), 24 sweep + 8 reference backtests. Frozen pick raises inner-train Sharpe over v4 (2.11 vs 2.03 spot, 2.36 vs 2.28 futures) while cutting DD 7-8pp, at an 8-12% return cost; roughly neutral on inner-validation. Holdout 2023+: **P1 FAILS** as predicted (never beats `buy_and_hold`, which v4 itself already loses to here). **P2 FAILS**: DD cut 1.9pp spot / 3.1pp futures, an order of magnitude under the 10pp bar; ΔSharpe ±0.01, inside the noise floor. Paired bootstrap (mean_block=30, n_boot=2000, 1,319 daily obs): Δlog-growth contains zero on both markets; Δmax-drawdown contains zero on spot (−1.68pp [−3.43,+1.00]) but **excludes zero on futures: −3.05pp [−5.34,−0.40]**, P(>0)=0.01. ETH falsification (Bitfinex, R-17/R-28/R-31 window), same frozen parameters, unrefit: does not falsify — BTC control 1.8pp spot/2.9pp futures, ETH test 3.7pp spot/**5.8pp futures**, same direction and comparable-to-larger magnitude, no exposure-matching step involved so this is not the R-31 trap.

Causality: by-hand check passes for all 6 variants (max diff before cut = 0.0 across `target`, `raw_target`, `dd_proxy`, `dd_multiplier`). **Skeptic: CONFIRMED** — traced the forward pass directly, confirmed `peak`/`dd`/multiplier at step `i` never reference step `i+1` or later, confirmed the one-bar execution lag, and independently reproduced the futures ΔmaxDD interval across 5 bootstrap seeds (1, 7, 42, 123, 999) with a stable point estimate and CI bounds — not a seed-lucky draw, and coherent with the smaller, same-signed spot estimate rather than looking like 1-in-4 noise, though it remains a single significant interval to weigh against the day's trials count.

**Promotion bar: P1 FAIL, P2 FAIL** (default reject regardless of P3/P4); **P3 PASS**; **P4 mixed** — a real plateau around "loose cap, convex power," but a sharp discontinuity toward the tight-linear variants, which showed the same shape as R-11's sells-low failure at smaller scale (inner-validation win, inner-train loss) and were deliberately not frozen for that reason.

**Verdict: NEGATIVE. Lesson:** calibrating the mechanism to be safe against R-11's failure mode (cap above v4's own ordinary drawdown range) makes it *inert* on the one holdout available — v4's 2023+ drawdown (27.8%/33.0%) never got close to threatening a 0.50 cap. Well-behaved (causal, plateau-friendly, no R-11-style collapse) but a negative of the "inert here" kind rather than the "actively harmful" kind. **Next step:** the value proposition is specifically about the tail. The 40-window Monte Carlo resample (R-19's design; `windows()` in `experiments/run_matched_risk.py` is a ready template) samples much deeper drawdowns than either the 2023+ bull holdout or the 2016-2019 ETH/BTC window and is the natural way to find out whether this mechanism does anything outside a regime that never stressed it, before shelving the idea. Filed as new backlog item **B-14**.

**Trials and holdout arithmetic for the round.** 6 configurations each, **12 total**, added to the project's running total: 172 (after R-32) + 12 = **184**. Holdout counter: R-33 read the 2023 holdout ~24 times by its own account (8 holdout, 4 interval, 12 falsify) plus ~2 in the skeptic's independent re-run; R-34 read the 2023+ holdout ~3 times (2 per-market evaluations, 1 interval computation reusing that same equity — R-30's "recovered from an answer already given" reasoning applies) plus ~1 in the skeptic's re-derivation (the 5-seed reproduction reuses the same daily-return series, not 5 fresh consultations). **~124 before, +24 (R-33) +4 (R-34) ≈ ~152.** Neither branch's holdout reads changed any decision rule after the fact — both froze before their first holdout read, per `git log` on the operator's own commit, and both verdicts are exactly what was predicted in the pre-registration.

**Neither branch is registered.** Per ROUTINE.md step 5, a NEGATIVE verdict gets a ledger row and code kept under `experiments/` (not auto-discovered) unless the negative is instructive enough to earn a table row on its own, the way `minority_oracle` or `game_switch` did. Both of these are incremental refinements of an already-registered, already-extensively-documented strategy (`kelly_regime_v4`) rather than a new mechanism class, so neither meets that bar; the README comparison table, `docs/STRATEGIES.md` and `docs/VALIDATION.md` are unchanged by this round.
...
**Reconciled 08-19 after R-33/R-34.** The operator dispatched a parallel round outside the backlog's own ranking: one branch answered **B-05** (funding gate), the other designed a **novel** SIZE mechanism (own-drawdown feedback control) not previously on the backlog at all. Both NEGATIVE, both skeptic-confirmed, neither registered. B-05 is closed for the reason stated in its row — it needs either a redesigned threshold or B-02's data extension before it can be tried again — and the new mechanism opens **B-14**, filed below. The order is otherwise unchanged: **B-13 is still the top actionable item**, unaffected by this round, and remains the cheapest experiment that could change what this project believes about its own headline result.
...
| ~~B-05~~ | ~~Funding as a gate on the existing strategy (stand flat in the top decile)~~ | COST | **DONE → R-33** | NEGATIVE. Implemented exactly as scoped; on the only holdout year the committed funding data allows (2023), the gate is bit-identical to plain `kelly_regime_v4` — its expanding top-decile threshold was set by 2021's blow-off and never re-crossed. Not a near-miss: paired bootstrap returns exact [0.000, 0.000]. Next: a bounded rolling-window threshold (untested), or B-02 to get a holdout worth reading. |
| **B-14** | Monte Carlo stress test (R-19's 40-window design) of the own-drawdown feedback multiplier from R-34 | SIZE | **NEXT** | R-34's mechanism cut futures holdout drawdown by a real but small 3.05pp [−5.34,−0.40] and was otherwise inert, because v4's 2023+ drawdown (28–33%) never approached the calibrated 0.50 cap. The mechanism is specifically about the tail; the 2023+ holdout and the 2016–2019 ETH window both stayed too shallow to test it properly. `windows()` in `experiments/run_matched_risk.py` is a ready template — cheap, no new data, answers whether the idea does anything before it is shelved. |
...
| 08-19 | ~152 | R-33/R-34: +~28 on top of R-32's ~124 — R-33's own ~24 (8 holdout, 4 interval, 12 falsify, all against the 2023-only funding-covered slice) plus ~2 in the skeptic's independent re-run, and R-34's ~3 (2 per-market holdout evaluations, 1 interval computation reusing that equity — R-30's "recovered from an answer already given" reasoning applies to the skeptic's 5-seed reproduction, which reused the same daily-return series rather than reading the holdout 5 more times) plus ~1 in its own skeptic pass. Trials count: 6 + 6 = 12, added to R-32's 172 → **184**. Both branches pre-registered and froze before their first holdout read (the operator's commit history records the code checked in before either branch's results were written up), and both verdicts landed exactly on their own stated predictions — nothing here is a surprise that needed the goalposts moved. |
