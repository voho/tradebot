...
### R-81 · 08-21 · NEGATIVE (both branches, stopped at Step A) — CoinMetrics exchange net-flow, an eighth structurally distinct INFO signal: lags the price-anchor gate on direction, and adds no incremental volatility-forecasting power over HAR-RV on the SIZE axis

**Direction.** Off-backlog (the ranked list holds nothing but methodology/infra items — B-38/B-39/B-32 per R-78's re-ranking, not a strategy-improvement direction; per R-80's own re-ranking, the next mechanism round needs "a genuinely new *kind* of information or error-control mechanism this project has not yet checked in any form"). Literature search (web) surfaced CoinMetrics' `FlowInExNtv`/`FlowOutExNtv` (native BTC/ETH moving onto/off known exchange addresses) as reachable, unrestricted on the free community tier — probed directly this session, confirmed clean (zero missing days, BTC 2016-01-01→, ETH 2019-01-01→). This is a capital-custody / selling-pressure flow, the mechanism behind CryptoQuant's/Glassnode's netflow products and studied in Ren, Wu & Liu (2024, arXiv:2411.06327) "Return and Volatility Forecasting Using On-Chain Flows in Cryptocurrency Markets", which reports (their words, intraday 1-6h horizons): BTC's own net inflow *generally lacks return-forecasting power* but *negatively forecasts volatility*; ETH's own net inflow negatively forecasts both. **Not a duplicate of any of the eight prior INFO-axis signals**: B-07/R-44 (network-activity counts — address/tx/hashrate, not a flow), R-54/55/58 (aggregate stablecoin *supply growth*, a liquidity-flow proxy one step removed from the traded asset, not the asset's own custody flow), R-73 (DVOL, a priced expectation), R-74 (MVRV, a valuation ratio), R-53/54 (VIX/DXY, an equity/FX spillover), R-75 (calendar/session timestamp structure), R-79 (halving-cycle phase), R-80 (a confidence signal derived from the strategy's own vote, not external data). Attacks **INFO** (conservative branch: a new capital-flow signal as a directional confirming vote) and **SIZE** (novel branch: the same data as a volatility-forecasting input feeding `kelly_regime_v4`'s scale term — not a duplicate of R-59/R-60's SIZE-axis retunes, which changed the *magnitude* and *timing* of the existing trailing-realized-vol input using only price-derived data; this is the first SIZE-axis round to bring in an orthogonal, price-independent forecasting input at all). **What would make it fail, named in advance** (matching the paper's own honest caveat that BTC's own net inflow lacks return power): the conservative branch's lead-time gate lags rather than leads, like DVOL/MVRV/halving-phase before it; the novel branch's incremental-R² gate is too small to matter once a HAR-RV baseline already explains most of next-day variance.

**What was done.** Operator-built shared infrastructure first (`scripts/fetch_exchange_flows.py`, `tradebot.data.load_exchange_flow`, `experiments/r81_shared.py` — episode list identical to R-73/74's, causal net-flow helpers, re-exports of R-80's anchor-vote/confirming-vote utilities so neither branch re-implements them), committed separately per this project's shared-utility convention (R-41/R-53/R-58/R-80). Two parallel, disjoint-file branches, neither touching the other's files or `r81_shared.py`, neither commiting past its own file. **Conservative** (`experiments/r81_conservative_leadtime.py`): pre-registered Step-A lead-time gate, run BEFORE any strategy code — a rolling-90-day z-score of BTC `net_flow` (`FlowInExNtv − FlowOutExNtv`), latched into a bear/calm vote (`THRESH_HI=1.0`, `GAP=0.75`, R-53/54/55/73/74's own hysteresis convention, fixed a priori), bear-onset dates compared to `kelly_regime_v4`'s own 3-anchor-majority bear onsets on the same 5 canonical episodes R-73/74 used (2018 bear, 2020-03 COVID, 2021-top/2022-bear, Terra/Luna, FTX), ±90-day window. Pre-registered rule: proceed only if a majority of matched episodes lead with positive median `lead_days`. **Novel** (`experiments/r81_novel_flowvol_gate.py`): pre-registered Step-A HAR-RV incremental-forecast gate, run BEFORE any strategy code — two OLS regressions on inner-train (2017–2020) forecasting `log(rv[t+1])` (daily realized vol = std of that day's 5m log returns) from a standard Corsi (2009) HAR baseline (`log(rv_1d)+log(rv_7d)+log(rv_30d)`) vs. baseline + `net_flow_z`, evaluated out-of-sample on inner-validation (2021–2022), coefficients frozen from the inner-train fit. Pre-registered rule: proceed only if candidate's OOS R² exceeds baseline's by ≥+0.01 AND the flow coefficient keeps its sign under a one-shot refit on inner-validation alone. **Round total: 3 configurations** (conservative: 0 strategy configs, a pure measurement gate; novel: 2 regression fits + 1 stability refit). Both branches' every frame is truncated at or before `2022-12-31` and independently `assert_no_holdout()`-guarded; the operator independently reproduced both from a clean shell (`python experiments/r81_conservative_leadtime.py leadtime`, `python experiments/r81_novel_flowvol_gate.py`) and every number below matched each branch's own report exactly.

**Result.** **Conservative**: 5/5 episodes matched within ±90 days, but only **1/5 led** (net-flow signal arrived first only for FTX, +3 days), with median `lead_days = −14.0` — the net-flow-bear vote lags the price-anchor majority by two weeks on the median episode (2018: −22d, COVID: −14d, 2021-top/2022-bear: −3d, Terra/Luna: −39d, FTX: +3d). Both pre-registered clauses (majority-leading, positive median) fail decisively, not marginally. Descriptively, the net-flow vote also flips far more often than the price anchors over the same window (84 bear-onset events vs. 40) — a noisier, faster signal being asked to lead a slower one, which is consistent with why it lags. **Novel**: baseline HAR-RV alone already reaches OOS R² = **+0.6033** on inner-validation (daily realized vol is strongly autocorrelated at this cadence); adding `net_flow_z` moves it to **+0.6054**, a delta of **+0.0021** against the pre-registered +0.01 bar — fails by roughly 5x. The flow coefficient is sign-stable (+0.0316 inner-train, +0.0266 inner-validation refit, both positive — the opposite sign from the paper's own *negative* volatility relationship, though far too small in either direction to matter) — check (2) passes, check (1) fails, and both are required. Neither branch wrote any strategy code past its gate; Step B was never reached by either.

**Verdict.** **NEGATIVE, both branches, stopped at Step A.** One-line lesson: this project has now run 9 lead-time/incremental-forecast gates against `kelly_regime_v4`'s own price-driven regime signal (8 prior INFO-axis attempts plus this one) and none has shown a price-independent signal leading or materially improving on it at the daily-to-multiday cadence this strategy actually re-evaluates on — the intraday (1-6h) effects the literature reports for exchange flow specifically do not survive translation to this project's cadence, on either the direction or the volatility-forecasting side of the same data. **Holdout counter: +0** — running program-level total stays **~637** (R-78/79/80's figure, unchanged); see the bullet added below in [Holdout consultations to date](#holdout-consultations-to-date). Neither branch's decision rule moved after being run. **Next step**: the backlog should now treat the lead-time-gate/incremental-forecast methodology itself as likely exhausted on the INFO axis for this strategy's daily cadence — a future INFO-axis attempt would need either a materially different question (not "does X lead the price-anchor gate") or a cadence-matched construction (the novel branch's own suggestion: forecast a few-hours-ahead volatility feeding a scale term that updates faster than once a day, which would require rearchitecting `kelly_regime_v4`'s update frequency to test honestly) rather than a tenth data source tested the same way nine have already failed.
...
**Re-ranked 08-21 after R-81.** An off-backlog, literature-prompted round (same posture as R-53/R-56/R-73–R-80 — the ranked list holds only methodology/infra items, not a mechanism direction) found and tested a genuinely new data source, CoinMetrics' free exchange net-flow (capital-custody/selling-pressure), on both axes R-80's own re-ranking asked for. Both branches NEGATIVE, both stopped at a pre-registered Step-A gate before any strategy code: the conservative branch's lead-time test found net-flow lags `kelly_regime_v4`'s own price-anchor regime flip by a median of 14 days (1/5 episodes leading, 5/5 matched) — the ninth INFO-axis signal to fail; the novel branch's HAR-RV incremental-forecast test found net-flow adds only +0.0021 out-of-sample R² against a pre-registered +0.01 bar, roughly 5x short — the first SIZE-axis round to test an external forecasting input at all, and it fails on power, not on mechanism (the flow coefficient is sign-stable, just too small to matter once HAR-RV's own autocorrelation already explains ~60% of next-day variance). Nothing new opens on the backlog. The order below is unaffected: **B-38 (a risk-matched forward comparison, filed and costed by R-78) remains the highest-value actionable item**; B-39 and B-32 stay next; nothing else is open. This is now the ninth consecutive off-backlog mechanism round (R-73–R-81) to fail, and the pattern is narrowing further: eight of nine failed at a pre-registered Step-A gate before any strategy code, and this round's novel branch shows the same discipline now extends cleanly onto the SIZE axis, not just direction/lead-time questions. A future session preferring a fresh mechanism search over the backlog's methodology items now needs either a genuinely different *kind* of question (not "does X lead/forecast the existing price-driven mechanism", which nine rounds have now asked of nine different signals with one consistent answer) or a cadence-matched construction R-81's novel branch names explicitly (an intraday volatility-forecast feeding a scale term that updates faster than once a day, which needs `kelly_regime_v4`'s update frequency rearchitected to test honestly) — or should work B-38/B-39/B-32 directly, per R-78's own re-ranking below.
...
|---|---|---|---|---|
| **B-38** | Pre-register and record a **risk-matched** forward comparison instead of the raw one B-06 has been recording: pair `kelly_regime_v4` against a passive long carrying v4's own mean notional (R-33's matched benchmark, per-window matched, not a fully-invested hold), so the paired daily difference stops carrying ~0.6-0.7 of BTC's own move as common-mode variance. Decide in advance what horizon would make it worth continuing | N≈3, ERR | **OPEN, highest-value actionable** | Filed by R-78, and **costed by its own addendum before filing** (`experiments/r78_matched_arm_sizing.py`): matching cuts the paired daily noise from 0.0320 to 0.0145 (−54.8%, inner-train) and 0.0304 to 0.0108 (−64.5%, inner-validation), and the look-once horizon from 808.8 → **42.1 years** on inner-validation (19×) but only 7.1 → 6.2 years on inner-train, because on a matched pair the mean shrinks alongside the noise. **The noise reduction is the reliable half; the horizon gain is regime-dependent and 42 years is still not viable** — which is the point of filing this as a pre-registered measurement rather than as a replacement plan. Its own named failure mode: the matched pair's *mean* may go to zero as fast as its variance, in which case the horizon does not improve at all and the honest conclusion is that no forward comparison of this family is settleable, which is itself worth knowing and is a NEGATIVE, not a null round. Needs no new data and reads no holdout. |
| **B-39** | Carry R-78's `level_resync_order()` fix into `bot.py` / `live_bot.py`, which still gate on the same edge-triggered `abs(target[i] − target[i−1]) > 1e-9` and therefore lose every target change that lands between two invocations for anyone running them slower than the bar interval | methodology (not one of the four constraints) | **OPEN, LOW effort** | Filed by R-78, which fixed the identical defect in `scripts/paper_trade.py` only, because those two files were not that round's to change (the same scoping R-71 used when it fixed the inception half). The measurement transfers unchanged: exactly `1/k` of a strategy's target changes survive a 1-in-`k` decision grid, so a live account polled every 15 minutes acts on ~1/3 of them and one polled hourly on ~1/12. Not urgent — nothing is trading live off these files today — but it is a known, measured, already-solved defect sitting in the deployment path this repo documents, and leaving it there while `docs/LIVE.md` tells operators a missed run "costs nothing but the missed rebalance" is the same wrong sentence R-78 just corrected in one file out of three. |
...
- **08-21 · ~637** — R-81: **+0** on top of R-80's ~637 (unchanged). Both
  branches' every frame is truncated at or before `2022-12-31` (inner-train
  + inner-validation only) and independently `assert_no_holdout()`-guarded;
  both are pure Step-A measurement gates that stopped before any strategy
  code, and therefore before any holdout-reading code path, was ever
  written. The operator's independent, clean-shell reproduction of both
  branches (`r81_conservative_leadtime.py leadtime`,
  `r81_novel_flowvol_gate.py`) printed no timestamp later than 2022-12-31
  and matched every reported number exactly. This round builds no
  `BTC_HOLD` cell and is not descended from `r63_shared.py`'s pattern, so
  B-33's standing instruction does not bite. See R-81 in section B.
