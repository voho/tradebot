...
| **B-17** | Multi-asset strategy registration — the comparison table, `Strategy` base class and `tradebot run` all assume one instrument per registered class; nothing in this project can put a genuinely two-asset (or N-asset) strategy in the README table today, even a fully-promotable one | ERR (methodology gap, not a market-constraint code) | OPEN | Surfaced by R-43 while writing that round's holdout pre-registration: the dual-asset BTC+ETH branches (R-42, R-43) are the first ten-plus rounds' worth of work that could not be registered even in principle without this. Lower priority than B-06 and than any *specific* dual-asset finding actually clearing a holdout (R-43's did not) — but worth scoping once such a finding exists, so a future confirmed result is not stuck outside the table for a purely mechanical reason. Not attempted this session. |
| **B-18** | Is the `kelly_regime_covkelly` allocator's monthly/weekly cadence-inconsistency (R-42, attenuated but not resolved by R-43's mean-denoising) actually a rebalance-engine/segment-restart artifact rather than a mean-estimation-noise problem? | ERR | LOW | Named as an open question by R-43's novel-branch author: since de-noising the mean shrank but did not eliminate the monthly-loses/weekly-wins flip, the remaining defect may be structural (each rebalance segment's `kelly_regime_v4` sub-book restarts flat/unlatched rather than carrying position forward — a documented simplification in both `kelly_regime_covkelly.py` and `_v2.py`) rather than about `mu`/`Sigma` at all. Worth checking before spending more effort on estimator variants. Below B-06 and B-17 on merit. |
