...
| R-33 | B-05 — funding as a gate on `kelly_regime_v4`: haircut long exposure when trailing funding sits in its own causal top decile (He et al. 2024; R-14, R-16) | 08-18 | `experiments/funding_gate.py`, `KellyRegimeFundingGate`. 13 configurations (quantile × trailing window × haircut) swept on inner-validation (2021–2022) with real funding charged. Frozen: `quantile=0.90, trailing=1d, haircut=0.50`. Pre-registered falsification: survive Bitstamp's 0.40% taker tier. | Only the 1-day trailing window beat ungated v4 in-sample (3d/7d never won) — a peak on that axis, a plateau only on the haircut axis within it. Holdout (the one year, 2023, with real funding to charge): spot **lost** to ungated v4 (−1.8%), futures barely won (+1.3%, inside the ±0.2 noise floor); max drawdown was bit-identical to ungated v4 in every row because **the gate fired on 0.55% of 2023's bars** — funding never returned to 2020–21 levels, so this was a near-null test of R-16's signal, not a refutation of it. Falsification **FAILED**: at 0.40% taker the gate is worse than ungated v4 ($2,355 vs $2,445), not better — its rare extra rebalances cost more without adding exposure benefit. P1/P2/P3 fail, P4 mixed. | **NEGATIVE** — default reject; B-05 closed |
| R-34 | Bayesian-shrinkage Kelly: continuous posterior-uncertainty discount on `kelly_regime_v4`'s exposure, vs R-28's sequential-test gate (MacLean–Thorp–Ziemba 2010; Sukhov 2026, SSRN; Busseti–Ryu–Boyd 2016) | 08-18 | `experiments/bayes_shrink_kelly.py`, `KellyRegimeBayesShrink`: κₜ from a Beta(a,b) posterior (exponential forgetting) over the anchor vote's own recent hit-rate, multiplying the v4 target before the deadband. Discovered along the way: the vote's true conditional hit-rate is **below 50% at 5-minute resolution** (49.92% vs 49.76% unconditional — the mechanism never trades there) and only recovers above it (52.95% vs 52.04%) at **daily** resolution, which the frozen design uses. 9 real configurations (half-life 5–180d) on inner-validation (18 backtests counting the discarded 5m-resolution sweep that never traded), frozen at `halflife=10d`. Pre-registered falsification: ETH drawdown replication, same convention as R-17/R-28. Pre-registered prediction (written before the holdout): P1 fails, for the same N≈3 reason R-28 failed. | **Prediction confirmed.** Holdout P1 **FAILS** as predicted (spot $1,247 vs holding's $3,839); reported the deepest drawdown cut in the project (spot 6.1%, futures 8.5%) and the ETH falsification **PASSED**, more strongly than R-28's own (BTC 9.0%/ETH 16.6% vs R-28's 14.4%/19.5%). An independent skeptic reproduced every headline number exactly, found no lookahead (a genuinely online recursive Beta update, no full-series fit, a full extra day of deliberate lag on the daily→5m broadcast, four independent causality cuts including two the primary never tried, all bit-identical before the cut), and confirmed the "never trades at 5m" finding is real, not a cover story. **But the drawdown claim did not survive a matched-exposure check**: `bayes_shrink_kelly` holds 0.14x v4's mean exposure (correlation with R-28's own E1 evidence gate: 0.767), and a **naive constant rescale of v4 alone** (k≈0.12, matching mean \|target\|) matched or beat it on drawdown and Sharpe on 3 of 4 market/metric cells. Same low-exposure-artifact pattern R-31 found in R-28's P3 claim, on a third, independently-built mechanism. | **NEGATIVE** — and a second independent confirmation of R-31's general rule (a risk-reduction claim is only about the mechanism at matched risk) |
...
**Re-ranked 08-18 after R-33/R-34.** A two-branch parallel round worked
B-05 (conservative) and an off-backlog idea (novel: Bayesian-shrinkage
Kelly sizing, chosen because it attacks ERR by a mechanism distinct from
R-28's sequential test, per the operator's literature search) plus an
independent skeptic pass on the surviving claim. Both branches are
**NEGATIVE**; B-05 is closed. The skeptic's finding is the one worth
carrying forward: R-34's headline drawdown number, like R-28's before it,
did not survive a matched-exposure check — a third independent
confirmation that this project's risk-reduction claims keep manufacturing
themselves out of low exposure unless checked at parity. That is the same
knife **B-13** points, and this round is further evidence B-13 should stay
at the top rather than be treated as adequately answered by R-31/R-32
alone (those checked the e-process gate; the vote gate itself, L-04's
original claim, is still unchecked at matched risk against a de-levered
hold). This round's trials: 13 (B-05) + 18 (R-34, including the discarded
5m-resolution sweep that never traded) = **31**, so the project applies
172 + 31 = **203** as the running total for any future deflated-Sharpe
calculation. Code for both branches lives in `experiments/funding_gate.py`
and `experiments/bayes_shrink_kelly.py` (not auto-discovered, per ROUTINE
step 5 — neither cleared the promotion bar, so neither is registered).
...
| ~~B-05~~ | ~~Funding as a gate on the existing strategy (stand flat in the top decile)~~ | COST | **DONE → R-33** | NEGATIVE. Only fired on 0.55% of the one holdout year with real funding to charge (2023) — funding never returned to 2020–21 levels, so this was a near-null test of R-16's signal rather than a refutation of it. Failed its own falsification test (worse than ungated v4 at the 0.40% fee tier). R-16's underlying quintile spread is not contradicted; re-testing this gate needs funding data past 2023, i.e. is blocked on B-02. |
...
| as of | count | note |
|---|---|---|
| 08-18 | ~135 | R-33/R-34: ~11 new reads of the 2023+ BTC holdout (R-33: frozen gate × 2 markets on the 2023 real-funding window + × 2 markets on the full no-funding holdout + 1 spot fee-tier re-run ≈ 5; R-34: frozen config × 2 markets, reproduced exactly by the skeptic × 2 markets, plus the skeptic's matched-exposure check × 2 markets ≈ 6). The ETH falsification runs (both branches) do not touch the 2023+ BTC holdout, per the R-17/R-28 convention. |
