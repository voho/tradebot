...
| **B-16** | Dual-asset BTC+ETH diversification of `kelly_regime_v4`, robustified per its own two authors' prescriptions | N≈3, SIZE | OPEN | Opened by R-42. Two next steps, one per branch: (1) bootstrap/path-resample the conservative fixed-split branch's inner-validation drawdown improvement (−4 to −7pp) — it currently rests on a single joint-bear window (2022), the same N≈3 fragility the branch set out to attack; (2) replace or shrink the novel covariance-aware branch's raw trailing-mean estimator (an equal-Sharpe prior, or a Σ-only mean-free weighting per MacLean/Thorp/Ziemba's own warning) and recheck for cadence-driven train/validation inconsistency before either earns a holdout read. Below B-06 on merit — B-06 remains uncontaminated evidence this dataset cannot offer; this is a further re-reading of data already in hand, but a genuinely new mechanism rather than an eleventh BTC-only-axis variant. |
