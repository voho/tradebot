...
| **B-19** | Does a periodically-rebalanced, EQUAL-WEIGHT (static 50/50) BTC+ETH portfolio of `kelly_regime_v4`, run through R-50's continuous (non-restarting) engine, survive pre-registration and this project's falsification/cost/holdout process? | SIZE, N≈3 | OPEN | Surfaced as a byproduct of R-50/B-18: at matched risk-free comparison, the fixed-50/50-continuous-engine portfolio beats v4-BTC-solo by ΔSharpe +0.79/+0.80 (monthly/weekly) with max DD improving from 33.2% to 27.1%, on inner-validation only. Not pre-registered, no falsification test run, no fee/funding sensitivity, no holdout read — genuinely promising but deliberately not rushed to promotion in the round that found it (this project's own standing rule against moving the goalposts after looking). Registering it for real would also need B-17's still-open "wire a multi-asset strategy into `run.py`/README/CI" half, since today's `src/tradebot/multiasset.py` adapter only supports a one-time fixed split, not R-50's periodic-rebalance-via-return-splicing design — that gap would need to be closed too, or the candidate re-expressed as a one-time-split adapter strategy and re-measured (a cheaper first check than building periodic rebalancing into `multiasset.py`). Now the highest-priority genuinely OPEN item on the backlog. |
