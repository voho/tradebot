...
**New data-quality flag from R-77 (08-21), orthogonal to the four constraints above — read this before trusting any short-horizon claim from this project's primary series.** This project's tradable BTC file (`data/btcusd_spot_5m.csv.gz`, Bitstamp-sourced) does not correlate with two independent BTC feeds (Coinbase, Bitfinex) at 5-minute-to-4-hour resolution (≈0, 2017 through mid-2023 against both) while its **daily** aggregates are correct (0.999 level / 0.81 return correlation vs. Bitfinex) and its index is monotonic, unique and gap-free. Found only because R-77 was the first round to need two independent same-instant series to agree, rather than merely trend together. Every `kelly_regime`-family finding, which sizes off daily-scale anchors, is unaffected; anything leaning on this file's *short-horizon* path shape (realized-vol sizing windows, `stress_test.py` window realism, any momentum/reversion signal) now carries an uninvestigated risk it did not carry before this round. See R-77 in section B and **B-38** in the backlog.
...
### R-77 · 08-21 · NEGATIVE (both branches) — cross-venue (Bitstamp vs Coinbase) BTC signal, the first INFO attempt using a second independent read of the SAME instrument rather than a different instrument or an external feed; the novel branch's own null result and the conservative branch's implausibly large "edge" both trace to the same root cause, found and confirmed by the operator: this project's primary Bitstamp series does not correlate with two independent BTC feeds at 5-minute-to-4-hour resolution, though its daily aggregates are correct

**Direction.** Off-backlog (nothing genuinely OPEN but B-06/B-24, the posture R-53/R-56/R-73/R-74/R-75/R-76 were all run from). Every prior INFO-axis round fed `kelly_regime_v4` either an external data source about a different quantity (on-chain B-07/R-44, macro R-53, stablecoin R-54/R-55/R-58, DVOL R-73, MVRV R-74, calendar R-75) or a relationship between two *different* instruments on the *same* venue (R-76's pairs trading). None had used a second, independent read of the *same* instrument's price from a *different* venue — this project's tradable BTC series (`data/btcusd_spot_5m.csv.gz`) has only ever been Bitstamp-sourced. R-76's own pre-registration flagged this explicitly as an unaddressed confound on its BTC leg ("a genuine cross-venue basis could add noise... not corrected for"), never tested as a signal in its own right until now. Constraint attacked: **INFO**. Citations: Makarov & Schoar (2020, *Journal of Financial Economics* 135(2):293-319 — persistent, large cross-exchange crypto price deviations that transaction costs alone cannot explain); Hasbrouck (1995, *J. Finance* 50(4)) and Gonzalo & Granger (1995, *J. Business & Econ. Stat.* 13(1)) (the classic information-share / component-share price-discovery framework); Putniņš (2013, *J. Financial Markets* — the Information Leadership Share, correcting IS/CS bias under unequal cross-venue noise); Alexander & Heck (2020, *J. Financial Stability* 50:100776 — studies Bitstamp and Coinbase specifically among regulated spot venues and finds price leadership between them is not fixed). Not a duplicate of R-41 (Deribit perp-vs-spot basis, a derivatives/spot basis on one venue), R-76 (cross-*instrument*, same venue), or R-39/B-05 (funding-rate cross-venue splice, a rate not a price).

**What was done.** A fresh fetch (`scripts/fetch_coinbase_spot.py --product BTC-USD --start 2017-01-01`, the same endpoint R-57's panel and R-39's ETH-basis series already used) added `data/btcusd_coinbase_spot_5m.csv.gz`, since Coinbase BTC had never been committed (only ETH and six other Coinbase assets had). The operator wrote and froze `experiments/r77_shared.py` before dispatching either branch: `load_crossvenue_bars()` returns the Bitstamp frame (the tradable series) restricted to timestamps Coinbase also has a bar, plus one causal column, `divergence = log(bitstamp_close / coinbase_close)`. Two parallel branches, disjoint files, neither committed by the branch itself (operator committed both as WIP snapshots once both reported): **conservative** (`experiments/r77_conservative_crossvenue_confirm.py`) reused R-55's validated precision-weighted confirming-vote architecture (`kelly_regime_v16_stablecoin_confirm.py`'s pattern) with `divergence`'s rolling z-score as a 4th vote, sign checked empirically on inner-train before freezing (negative at all 12 window×horizon cells — "Bitstamp cheap → bullish" the Makarov & Schoar arbitrage-friction direction). **Novel** (`experiments/r77_novel_crossvenue_leadlag.py`) built a causal rolling partial correlation of `bitstamp_return[t]` on `coinbase_return[t-1]` controlling for `bitstamp_return[t-1]` (Hasbrouck/Putniņš-style leadership, lagged by construction to avoid same-bar lookahead) as its own pre-registered falsification gate, run *before* any strategy code. **Configs evaluated: 17** (conservative: 1 identity control + 4×4 window/threshold/weight sweep, inner-train + inner-validation, both markets) **+ 0** (novel: gate failed before any sweep) **= 17 total**. `pytest` 492/492 passed after both branches (no `src/` file touched, no strategy registered by either).

**Result.** Novel branch: **NEGATIVE at the pre-registered gate.** The lead-lag statistic is statistically indistinguishable from zero in inner-train (static lag-1 partial r = -0.00005, |r|/SE = 0.03) and remains small and lag-inconsistent in inner-validation (only 3/6 robustness lags agree in sign) — Coinbase's lagged return does not predict Bitstamp's current return beyond Bitstamp's own momentum. Per its own pre-registration, zero strategy configurations were swept and the holdout was never read. Its causality probes (synthetic known-lag recovery, truncation invariance, identity check) all passed, confirming the null is a real measurement, not a broken test.

Conservative branch: its sweep initially looked like a strong win — inner-validation Sharpe ~5.8-6.0 with drawdown cut to ~11% (spot final balance $22.7K-$25.2K vs v4's $998 over 2021-2022), clearing the mechanical gate and passing the standard exposure-artifact regression (R² = 0.42, well under the 0.95 discard line) and its own causality tamper-probe. The branch itself did not stop there: per ROUTINE.md's own standing line ("a result that looks too good is a bug report first"), it checked the premise the whole mechanism depends on — that Bitstamp and Coinbase, two feeds of the same actively-arbitraged instrument, should correlate strongly at 5-minute resolution — and found they do not (correlation -0.011 to +0.004 in every year 2017-2022), refusing to read the holdout and reporting **BLOCKED** rather than a win.

**The operator independently verified this rather than taking either branch's word for it** (ROUTINE.md's "an independent skeptic re-runs each surviving claim... hunting specifically for full-series fits" — here adapted to a claim severe enough to kill the whole round rather than one that survived). Findings, in the order that isolated the cause:
1. Same-bar Bitstamp/Coinbase 5-min return correlation is ~0 for 2017 through mid-2023, then jumps sharply to 0.97+ from September 2024 onward (monthly series: 2024-08 = 0.007, 2024-09 = 0.415, 2024-10 = 0.971).
2. A live re-fetch of Coinbase's public candles endpoint for a 2020-06-15 window matched the committed file exactly — ruling out a fetch/paging bug on the Coinbase side.
3. **The Coinbase side is not the problem.** Coinbase-vs-Bitfinex correlation (two venues *neither* of which is Bitstamp, using the already-committed `ethusd_bitfinex_5m`/`btcusd_bitfinex_5m` control files) is 0.4-0.93 in the *same* 2017-2018 months where Bitstamp shows ~0 against both other venues — and the already-committed ETH Coinbase-vs-Bitfinex comparison (R-39/B-15's series) correlates at 0.81-0.95 throughout 2019. Coinbase's historical data is fine; **Bitstamp's is the outlier**.
4. Resolution scan (Bitstamp vs Bitfinex, 2017-2018 overlap): 15-min/1h/4h return correlation ≈ 0 or slightly negative (-0.0023 to -0.024), but **daily-resampled return correlation is 0.81 and daily close-level correlation is 0.999**. The index is monotonic, unique, and gap-free against a full 5-min grid (0 missing bars out of 1,010,889) — this is not a sort or index-corruption bug. Bitstamp's 2017-06 lag-1 return autocorrelation (-0.093) is roughly 25-35x Coinbase's and Bitfinex's own (-0.003, -0.004) in the same month.

**Scope of this finding, stated precisely rather than over-claimed.** This project's tradable BTC series (`data/btcusd_spot_5m.csv.gz`, built by `scripts/build_bitstamp_dataset.py` from `github.com/ff137/bitstamp-btcusd-minute-data`) carries correct daily-level price history — the multi-decade bull/bear structure every `kelly_regime`-family anchor (20-180 day rolling means, order-invariant to any within-window reshuffling) depends on is not in question, and neither is the causality-test suite (row order is monotonic; this is a values problem, not a lookahead problem). What is now in question, for the first time in 77 rounds, is whether this file's *sub-daily* (5-minute to few-hour) dynamics reflect genuine synchronized trading at all: no prior round had a reason to check, because no prior round needed a second, independent, same-instant series to compare against. This directly explains both branches' results — the novel branch's true-zero lead-lag measurement and the conservative branch's implausible Sharpe are the same artifact viewed two ways, and it also means R-77's entire premise (that Bitstamp/Coinbase divergence reflects arbitrage friction or information leadership) cannot be evaluated on this data as constructed. It does **not**, on current evidence, cast doubt on findings that depend only on Bitstamp's own daily-level trend (the whole `kelly_regime` family's core mechanism) — but it is a new, concrete reason to treat any *other* past or future finding that leans on Bitstamp's realized short-horizon volatility or path shape (the `vol_span=8*BARS_PER_DAY` EWM every `kelly_regime` variant sizes against, `stress_test.py`'s window realism, every momentum/reversion strategy's short-horizon signal) as carrying an uninvestigated data-quality risk it did not carry before this round.

**Verdict.** **NEGATIVE, both branches.** Novel: clean null at its own pre-registered gate, holdout never read, 0 configs swept. Conservative: **BLOCKED** — its mechanically-passing result is not believed, on the branch's own diagnosis and the operator's independent confirmation; holdout never read for any strategy decision (`holdout()` exists in the file as a subcommand per this round's brief but was never invoked — confirmed by reading `all_checks()`, which stops at the gate-check print and does not call it). **Holdout counter: +1 descriptive consultation, running total ~638 (up from R-76's ~637).** Neither branch's own code read 2023+ data. The operator's own diagnostic work verifying the conservative branch's data-quality claim did, however, read raw price correlations from the full 2017-2026 committed series — a same-bar Bitstamp-vs-Coinbase correlation computed year-by-year and by month across 2023-2026, which is what located the September 2024 transition. This is a descriptive statistic about the *data itself* (return correlation between two price feeds), not an out-of-sample read of any strategy's Sharpe, drawdown, or PnL, and it supports no promotion decision — the same category R-33's own re-match fell into ("labelled in-sample everywhere it appears and supports no decision") — but it is still a real holdout-period read and is counted rather than glossed over, per this file's own standing discipline that looking is not what corrupts a holdout, hiding it is. One-line lesson: a cross-venue signal is only as good as its two feeds' genuine independence, and this project had never had a reason to check that until a round finally needed two feeds to agree at 5-minute resolution rather than merely trend together. **Next step, filed as B-38 (see backlog):** a dedicated data-audit round should re-source Bitstamp BTC directly (not via the `ff137` community mirror) for a sample window and compare against Coinbase/Bitfinex at sub-daily resolution before any future round leans on this project's primary series' short-horizon path shape; until then, `r77_shared.py`, both branches' files, and the new Coinbase BTC file stay in `experiments/`/`data/` as a documented negative result plus the evidence behind it, not retried on this construction.
...
**Re-ranked 08-21 after R-77, which opens a new item ahead of everything else.** A cross-venue (Bitstamp vs Coinbase) BTC signal — the first INFO attempt using a second independent read of the *same* instrument rather than a different instrument (R-76) or an external feed (R-44/R-53/R-54/R-55/R-58/R-73/R-74/R-75) — came back NEGATIVE on both branches, but not for the usual reasons. The novel (lead-lag) branch found a clean null (Coinbase's lagged return does not predict Bitstamp's, statistically indistinguishable from zero in inner-train) and the conservative (confirming-vote) branch found what looked like this project's best result ever (inner-validation Sharpe ~6, drawdown ~11%) — until it checked, and the operator independently confirmed, that the premise underneath both results is broken: **this project's primary Bitstamp series does not correlate with two independent BTC feeds (Coinbase, Bitfinex) at 5-minute-to-4-hour resolution** (correlation ≈0, sometimes negative, 2017 through mid-2023 against both), while its **daily** aggregates are correct (0.999 level, 0.81 return correlation against Bitfinex) and its index is monotonic, unique and gap-free. No prior round had a reason to check this — 76 rounds of `kelly_regime`-family sizing depend only on daily-scale trend, which this finding does not call into question — but it is a newly-discovered, uninvestigated data-quality risk for anything leaning on Bitstamp's *short-horizon* path shape (realized volatility sizing, `stress_test.py` window realism, every momentum/reversion strategy's signal). Filed as **B-38, top of the ranked list**: a dedicated data-audit round should re-source Bitstamp BTC independently of the `ff137` community mirror this project's file derives from, and compare it against Coinbase/Bitfinex at sub-daily resolution, before any future round leans on this series' short-horizon dynamics — cross-venue or otherwise. Not urgent enough to block **B-06** (forward paper trading, still ongoing, still zero-cost, still the standing recommendation on merit — and notably immune to this finding, since it trades the real live feed going forward rather than replaying this file's disputed history) but it is now a materially cheaper, more consequential item than a ninth INFO-axis variant or a fourth pairs-trading construction: it could change how much of this project's *existing* 76 rounds should be trusted, not just add one more. **B-24 stays LOW priority.**
...
- **08-21 · ~638** — R-77: **+1** on top of R-76's ~637 — not a strategy
  decision read: neither branch's own code ever read 2023+ data (the
  novel branch's pre-registered gate failed before any sweep; the
  conservative branch's gate mechanically passed but the branch itself
  stopped short of holdout on its own data-quality diagnosis, and its
  file's `holdout()` subcommand is never called by `all_checks()` —
  confirmed by reading it). The +1 is the operator's own diagnostic work:
  verifying the conservative branch's claim required computing same-bar
  Bitstamp-vs-Coinbase return correlation year-by-year and by month
  across the full committed series, which includes 2023-2026 and is what
  located the September 2024 correlation-recovery transition. A
  descriptive statistic about the data itself, not an out-of-sample read
  of any strategy's Sharpe/drawdown/PnL, and it supports no promotion
  decision (the same category as R-33's own re-match) — but it is a real
  holdout-period read and this file counts it rather than omitting it.
