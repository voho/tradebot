...
| **B-21** | A hard, unweighted macro-veto (`frac=0` while VIX/DXY `stress_z` is above threshold, v4's own anchor average otherwise — no precision-weighted averaging) as a `kelly_regime_v4` regime-gate override | INFO, SIZE | OPEN | Surfaced only as R-53 novel's ablation comparison arm, not its pre-registered candidate: on inner-validation it beat v4 outright (spot Sharpe 0.34 vs 0.14, DD 26.4% vs 33.2%; futures Sharpe 0.39 vs 0.25, DD 27.4% vs 32.3%) where the pre-registered precision-weighted-average version failed decisively. Untested by any branch: its own lead-time check (does the hard veto actually fire before the price-anchor gate, given the averaged version's vote was found to lag on net), ETH falsification, causality probe as its own subject, and parameter-neighbourhood plateau. A future session should pre-register a falsification test for this specific mechanism before running anything further — R-53's own lead-time finding (macro stress lags the 3-anchor majority by a median 5.5 days across the available episodes) is a real tension for a mechanism whose whole value proposition is faster gate-flipping, and should be resolved rather than assumed away by the blunter combination rule. |
