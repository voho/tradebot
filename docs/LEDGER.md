...
| R-33 | Funding gate on `kelly_regime_v4` (backlog B-05): haircut exposure when trailing perp funding sits above its own causal upper quantile (Cardaliaguet & Lehalle 2018; R-14; R-16) | 08-19 | `experiments/funding_gate.py`, one conservative branch of a two-branch parallel round. 13 configurations (span × quantile × haircut, one-knob-at-a-time) on the inner split, both markets. Causality self-checked (truncation + two-opposite-tampers, both PASS) and independently reproduced by the operator bit-for-bit. Frozen config re-run once on the 2023+ holdout per the pre-registered rule below. | **P2 and P4 fail.** Max drawdown is unchanged to four significant figures on both spot (27.8% vs 27.8%) and funding-charged futures (37.8% vs 37.8%) — the in-sample plateau's drawdown improvement does not replicate. Return improves (Δ log growth +0.047 spot, +0.096 futures, P(gate>v4)=0.91 both) but both 95% intervals graze zero ([−0.0008,+0.136], [−0.0005,+0.271]) — **≈**, not established, by this project's own convention. Neither strategy beats `buy_and_hold` on spot (P4). Reason found: the committed funding data (2020–2023) covers only the first ~27% of the 2023+ holdout — the gate is a no-op by construction for the other ~73%. | **NOT PROMOTED** — the in-sample finding was itself an artifact of funding-data coverage, not of the mechanism |
| R-34 | Crowding cost as a `kelly_regime_v4` sizing haircut, not a direction signal — the gap L-12 named explicitly (Cardaliaguet & Lehalle 2018; Harsanyi 1967-68) | 08-19 | `experiments/crowd_sizing.py`, the novel branch of the same parallel round. `harsanyi_crowd`'s trend-age × volume-efficiency-decay crowding score, re-keyed to `kelly_regime_v4`'s own vote sign, applied as `target *= (1 - lam_crowd*crowd)`. 24 configurations (`lam_crowd`×`age_scale`) on inner-train, 6-point shortlist re-scored on inner-validation, both markets. Causality self-checked, both probes PASS. | Return degrades **monotonically** in `lam_crowd` and in `1/age_scale` on **every one of 24 grid points, on both markets** — no interior optimum. The only two cells with a real drawdown improvement (futures, `lam≥0.7, age=20d`) give it up 61–73% of return for 9–10pp of DD. Every config gentle enough to preserve return (the plateau near `lam=0.3, age=80d`) is statistically indistinguishable from `kelly_regime_v4` on inner-validation (final balance ±6%, DD ±1pp, Sharpe ±0.14) — the mechanism does not both move the strategy *and* help it anywhere in the tested space. | **NEGATIVE, decided on the inner split — no holdout consultation spent.** See below for why. |

### R-33/R-34 — a two-branch parallel round on `kelly_regime_v4`, one conservative and one novel

Both branches follow ROUTINE.md's "running directions in parallel" section: disjoint files (`experiments/funding_gate.py` / `experiments/crowd_sizing.py`), neither branch committed, both report including the dead one, and the trials count below is the **total across both branches**, not per branch.

**Step 1, R-33 (conservative).** Attacks **COST** — R-14 measured perp funding running +20.05%/yr while `kelly_regime_v4` holds vs +2.78%/yr while flat, and R-16 measured a +3.57pp 14-day forward-return spread between funding quintiles; neither was ever turned into an actual gate. Not a duplicate of L-05/L-06 (fee-based EV deadband — a *trading*-fee no-trade band, not a *funding* signal) or R-15/R-16 (correlational only). Simulable: yes, using only the committed real 2020–2023 Binance funding file, defaulting to fully inactive outside that window (no extrapolation). Named failure mode: flattening on high funding removes upside during a continuing bull leg, since R-16 itself found high funding predicts negative returns only when price is *not* also rising, and its middle quintiles were non-monotone.

**Step 1, R-34 (novel).** Attacks **SIZE** — the one constraint this project has repeatedly found actually works. Not a duplicate of L-01–L-04 (the `kelly_regime` family never adds an independent crowding term) or L-12 `harsanyi_crowd` (which used the identical crowding score as a *direction* haircut on its own belief margin, not grafted onto the incumbent's exposure) or R-11 Grossman–Zhou (a drawdown-cushion mechanism, not a trend-age/volume-efficiency one). Simulable: yes, entirely OHLCV-causal, reusing `harsanyi_crowd.py`'s already-causal primitives. Named failure mode, informed by a very recent and explicitly caveated preprint (Lee 2025, arXiv:2512.11913, **withdrawn by its author for insufficient empirical validation** — treated here as a reason to design the falsification test carefully, not as a citation to lean on): crowded *momentum* factors reportedly show markedly lower crash risk than crowded reversal factors, so a crowding haircut may be actively unhelpful for a trend-following strategy like `kelly_regime_v4`.

**Step 2/3, both branches.** Iterated only against `end="2020-12-31"` (inner-train) and `start="2021-01-01", end="2022-12-31"` (inner-validation); neither branch read `start="2023-01-01"` or later at any point — confirmed in each branch's own report and, for R-33's frozen configuration, independently reproduced bit-for-bit by the operator (`FundingGateV4(settlement_span=5.0, funding_quantile=0.80, haircut=0.3)` on inner-validation: spot $1,127/+12.7%/DD30.0%/Sharpe0.37, futures $1,166/+16.6%/DD30.4%/Sharpe0.44 — identical to the branch's own reported figures). Both branches ran the same two by-hand causality probes (`experiments/matched_risk.py`'s convention: truncation, and two-opposite-tampers on the tail) since unregistered code gets no CI protection; both PASS.

**Configurations evaluated, this round: R-33 13 + R-34 24 = 37.** Combined with the project total through R-32 (172), the running total is **209**.

**R-34: decided without spending a holdout read.** ROUTINE.md step 4 evaluates a frozen configuration on the holdout; it does not require doing so when step 3's own inner-validation selection has already failed decisively enough that a holdout read cannot inform the decision. That is the case here: across the full 24-point `(lam_crowd, age_scale)` grid, on *both* markets, return degrades monotonically as the haircut strengthens, with no interior optimum — a smoking-gun pattern this project has not seen from a genuinely mixed result before (contrast R-28, R-31, R-32, all of which needed a holdout read to resolve a real ambiguity). Spending one of the project's dwindling holdout consultations (~124 as of R-32, on a dataset R-29/R-30 already showed cannot support a Sharpe-based claim) on a configuration whose own inner-validation numbers are statistically indistinguishable from not touching the incumbent at all would add a consultation without adding information — precisely the waste the backlog's own recent entries (B-11, B-13 notes) have been trying to avoid. Filed **NEGATIVE** on the inner-split evidence alone, per ROUTINE.md step 5's allowance to record a negative under `experiments/` without registering it. The falsification test that *would* have been used (ETH cross-asset, chosen over a fee/Monte-Carlo test because the open question is whether the crowding cost is a real transferable market property or a single-asset artifact — directly answering the Lee-preprint caution above) was never run, for the same reason.

**R-33 pre-registration — written and committed before the holdout is read.**

**Idea.** `FundingGateV4`: `kelly_regime_v4`'s own exposure, multiplied by a haircut whenever a trailing EWM-smoothed (span = 5 settlements) perpetual funding rate exceeds its own causal, expanding 80th-percentile threshold (`expanding(min_periods=60).quantile(0.80)` over settlements, not bars — never a whole-series quantile applied to early rows). Outside the committed 2020-01–2023-12 funding window the gate is fully inactive by construction (haircut multiplier = 1.0), verified directly: zero bars outside `[2020-01-01 03:00, 2023-12-31 19:00]` UTC ever have `funding_gate_active = True`.

**Frozen configuration.** `FundingGateV4(settlement_span=5.0, funding_quantile=0.80, haircut=0.3)` — the soft haircut, not the numerically-best hard gate (`hc=0.0`), chosen for three stated reasons: (1) turnover — `hc=0.3` trades 52 times on inner-validation vs `hc=0.0`'s 73, and this project's own L-05/L-12/R-12 lessons are specifically about apparent edges that are actually turnover-financed; (2) R-16's own middle-quintile non-monotonicity is a reason to prefer a graceful haircut over a hard 0/1 classification on a noisy signal; (3) plateau, not peak — every cell in `span∈{3,5} × q∈{0.80,0.85} × hc∈{0.0,0.3,0.5}` beats the inner-validation baseline on both markets on both return and drawdown, and `hc=0.3` sits centrally in that region. Neighbours that *fail* to clear the baseline, marking the plateau's edge: `span=10` (washes out — nearly flat vs baseline) and `q=0.75` (worse than baseline on spot — ordinary-bar noise starts firing the gate).

**Decision rule — promote only if all hold:**

- **P1 (cost).** On futures with funding **charged** (real rate through 2023-12-31, `scripts/funding_study.py`'s `_blended()` convention — real where observed, empirical mean elsewhere — for the remainder of the holdout), `FundingGateV4`'s final balance is not more than 5% below `kelly_regime_v4`'s funding-charged final balance over the same window.
- **P2 (drawdown).** Max drawdown improves over `kelly_regime_v4` by **≥ 1.5 percentage points on both spot and funding-charged futures** — set below the smallest neighbourhood improvement actually measured in-sample (1.9pp futures / 3.2pp spot on inner-validation), so the bar is real rather than restating what was already observed.
- **P3 (noise floor).** Spot Sharpe is not worse than `kelly_regime_v4`'s by more than the project's **±0.2 Sharpe noise floor** (R-20).
- **P4 (base bar).** Beats `buy_and_hold` on spot final balance, at the real fee tier.
- **P5 (falsification).** Survives a 0.40% taker fee on spot (the fee tier, not ETH — the committed data has no second asset's funding series, and applying BTC's funding regime labels to ETH price would test a different, weaker claim than this mechanism actually makes; a genuine second-asset test is blocked on the same network restriction as B-02/B-07/B-08).
- **P6 (no re-tuning).** The frozen configuration above is exactly what gets run on the holdout — no threshold search against holdout numbers.

Anything else is `NEGATIVE`, per ROUTINE.md's default-reject bar.

**Stated prediction before looking:** the in-sample effect is concentrated entirely in 2021–2022 (inner-train shows a zero drawdown effect, since real funding data only starts 20 days into a 4-year window dominated by the pre-2020 history) — a pattern consistent with either "the gate correctly avoided 2021's blow-off top and 2022's crowded-long unwind" or "the funding window happens to overlap almost exactly with BTC's two worst crowded-long drawdowns of the decade, and this is N≈3 again in a new costume." Expect P1 and P4 to pass (the config is close to v4, which already beats holding most of the time) and treat P2 as the real test — genuinely uncertain which way it goes given the small number of distinct crowding episodes the funding data actually contains.

### R-33 results — the drawdown finding did not replicate, and the reason is legible

**Holdout, 2023-01-01 → 2026-08-12, $1,000 start.** `python experiments/run_funding_gate.py holdout`, one call, frozen config only, no threshold search.

| | spot final | spot DD | spot Sharpe | fut5x (funding-charged) final | fut5x DD | @0.40% spot final |
|---|---|---|---|---|---|---|
| `buy_and_hold` | **$3,839** | 54.0% | 1.03 | — | — | $3,827 |
| `kelly_regime_v4` | $3,373 | 27.8% | 1.22 | $3,133 (funding paid $1,190) | 37.8% | $2,445 |
| `funding_gate_v4` (frozen) | $3,536 | **27.8%** | 1.27 | $3,505 (funding paid $1,265) | **37.8%** | $2,508 |

Paired block bootstrap (gate − v4, identical resamples, 30-day mean block, 2,000 draws): Δ log growth spot **+0.047 [−0.0008, +0.136]**, futures **+0.096 [−0.0005, +0.271]** — both graze zero, P(gate>v4) = 0.91 in each, **≈** by this table's own ▲/≈/▼ rule, not ▲. Δ max drawdown: spot **−0.0000pp [−1.86, +0.002]**, futures **+0.0000pp [−3.34, +0.043]** — no effect, full stop. Deflation breakeven sd at the project's 209-trial cumulative count: 0.15 (spot) / 0.22 (futures) — far below any dispersion this project's actual sweeps have shown (R-28's 24-trial search alone already put a much larger Sharpe below the deflated bar), so the observed Sharpe advantage does not survive trials adjustment either.

**Decision, against the pre-registered rule:**

- **P1 PASS** — funding-charged futures final ($3,505) is *above* `kelly_regime_v4`'s ($3,133), not merely within 5%.
- **P2 FAIL** — required ≥1.5pp drawdown improvement on both markets; measured improvement is 0.0pp on both. The in-sample plateau (1.9–3.2pp on inner-validation) **did not replicate**.
- **P3 PASS (moot)** — spot Sharpe is better, not worse, than v4's.
- **P4 FAIL** — neither `funding_gate_v4` ($3,536) nor `kelly_regime_v4` ($3,373) beats `buy_and_hold` ($3,839) on spot — the same finding R-29/R-30 already established for the whole comparison table, inherited unchanged here.
- **P6** — no re-tuning; the frozen config from the pre-registration is exactly what was run.

**All hold required; P2 and P4 fail. NOT PROMOTED.**

**Why, and it is legible rather than mysterious.** `FundingGateV4` can only act where the committed funding file has data: 2020-01-01 → 2023-12-31. The 2023+ holdout runs to 2026-08-12 — **the gate is active for only the first ~53 weeks of a ~196-week holdout (≈27%)**, identical to plain `kelly_regime_v4` for the remaining ≈73% by construction (verified in R-33's causality check: `funding_gate_active` is `False` outside the file's exact date range). Both strategies show 51 trades on the holdout — the gate changed *position sizes* at a handful of points inside 2023, not the trade-timing pattern — enough to move return by a few percent but not enough to touch a max-drawdown statistic computed over the whole multi-year window. The in-sample plateau found in step 3 was real *on inner-validation* (2021–2022, fully inside the funding file's range) but was never going to transfer whole to a holdout three-quarters of which the mechanism cannot see. This is the same lesson B-02 already names from a different angle — the funding series stops in 2023 and extending it is blocked on network access — stated here as a direct, measured consequence rather than an inferred one.

**Secondary question, answered:** does a funding gate cut cost the way it was designed to? **Yes, unambiguously** — P1 passes clearly and funding-charged futures return improves, which is the one part of the pre-registered story that worked exactly as predicted. It just is not enough, alone, to clear the promotion bar without the drawdown or the buy-and-hold-beating legs.
...
**Re-ranked 08-19 after R-33/R-34.** B-05 is done and negative: the funding
gate improved return and cost on the holdout but produced **zero**
drawdown improvement, because the committed funding file covers only the
first ~27% of the 2023+ holdout — the mechanism is a no-op for the rest by
construction. That is a *data-coverage* finding, not a mechanism finding,
and it sharpens rather than dilutes **B-02** (extend the funding series
through 2026): B-02 is now not just "the cheapest item that could change a
decision" but the specific, named blocker on ever re-testing R-33 with the
holdout the mechanism could actually act on. The novel branch, R-34
(crowding cost as a `kelly_regime_v4` sizing haircut), closed on the inner
split without spending a holdout read — 24 configurations showed
monotonic degradation with no interior optimum, decisive enough that a
holdout consultation could not have changed the verdict. **B-13 remains
the top-ranked actionable item** (unchanged by this round, since neither
branch touched it), followed by B-05's replacement question — there is
none; B-05 is closed — so **B-05 no longer appears below**, and the next
computation-only item after B-13 is whatever a future session selects
fresh, since B-09/B-10 remain LOW and everything else actionable is
BLOCKED (network). Trials this round: R-33 13 + R-34 24 = 37, project
cumulative **209**. Holdout counter: **~135** (R-32's ~124 + 11 new reads
in R-33; R-34 spent none).
...
| ~~B-05~~ | ~~Funding as a gate on the existing strategy (stand flat in the top decile)~~ | COST | **DONE → R-33** | NEGATIVE on the promotion bar. P1 (cost) passed cleanly and return improved, but P2 (drawdown) and P4 (beats holding) both failed — the drawdown improvement seen on inner-validation produced **zero** effect on the holdout because the committed funding file covers only ~27% of it. Re-opens as a live question only if B-02 is unblocked. |
| **B-02** | Extend the funding series through 2026 | COST | **BLOCKED (network)** | The single cheapest item that could change a decision, sharpened by R-33: it is now the specific, named blocker on ever re-testing the funding gate against a holdout the mechanism can actually act on, not just a general data-freshness concern. Binance is unreachable from these sessions. Needs the operator. |
...
| 08-19 | ~135 | R-33: +11 on top of R-32's ~124 — one frozen config, 6 funding-free reads (3 strategies × 2 markets), 2 funding-charged futures reads, 3 spot @0.40%-fee reads; the bootstrap re-reads reran the same already-read (strategy, market, window) pairs and are not counted again, the R-30 convention. R-34 spent none (decided on the inner split — see the note above explaining why). Project trials from here: 172 + 37 (13+24, both branches of this round) = **209**. |
