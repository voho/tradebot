...
### R-78 · 08-21 · NEGATIVE (both branches) — closes R-77's own named open question: a directional-asymmetry execution rule (de-risking orders always fill immediately) eliminates R-56's crash-transition-lag danger completely, even at 12x its N≥72 threshold; a rescoped continuous adaptive-urgency throttle inherits the same danger unchanged; neither moves `kelly_regime_v4`'s Sharpe or drawdown past the noise floor

**Direction.** Attacks **COST** (costs that scale with the signal), off-backlog — the ranked list holds nothing genuinely `OPEN` but B-06 (ongoing/zero-cost). This round works the one specific, non-duplicate open question R-77 itself named in its own "next step": *"whether adaptive execution urgency helps specifically in the N≥72 region R-56 found genuinely dangerous, which no round (R-56, B-24, or R-77's novel branch) has yet tested an adaptive mechanism against."* Not a duplicate of R-56 (`experiments/kelly_regime_exec_limit_conservative.py`/`_novel.py`, fixed N up to 288, no adaptivity), R-77 conservative (B-24, fixed N capped at 24), or R-77 novel (regime-adaptive urgency, but `n_base` capped at 24 and its own fixed-N comparison arm never exceeded N=24) — see section C, both R-77 rows.

Two parallel branches, disjoint files (`experiments/r78_conservative_adaptive_n72.py`, `experiments/r78_novel_execution_directional_asymmetry.py`), neither committed by the branch itself — the operator committed both as a WIP snapshot once the first had reported (the second was already mid-run), then this write-up, per this project's established parallel-round convention.

**Conservative branch — R-77's own continuous adaptive-urgency formula, unchanged, `n_base` rescoped into the true danger zone.** Mechanism unchanged from R-77 novel: `N(i) = max(n_min, round(n_base/(1+κ·max(0,ratio[i]−1))))`, with an immediate-taker override for de-risking orders once the causal stress ratio (v4's own realized-vol/slow-EWM ratio) clears `s_override`. Only the swept range changes: `n_base ∈ {72,144,216,288}` (R-56's own original ceiling) `× κ ∈ {0.5,1.0}`, `s_override=1.70` and `n_min=2` fixed at R-77's own Stage-B/C winners (not re-tuned — this round's question is scope, not re-deriving the override threshold). 8 core sweep configurations. Pre-registered promotion rule, mirroring R-77 conservative verbatim plus the round's own thesis criterion: PROMOTE only if (a) beats v4's unmodified taker execution on inner-validation Sharpe by >±0.2, or a clear DD/tail win, both markets; (b) ETH falsification passes directionally; (c) BTC pre-2020 control doesn't decisively fail; (d) **the thesis** — crash-transition-lag mean ≤1-2 bars with no systematic blowup now that `n_base` genuinely reaches 72-288, tested as: adaptive's violation count ≥50% lower than a fee-savings-matched fixed-N baseline's, and mean lag no worse; (e) the `n_base × κ` neighbourhood is a plateau. **49 configurations** (4 causality + 9 sweep + 16 validation + 8 falsification + 1 crash-lag + 11 thesis-test, one clean end-to-end run).

**Novel branch — directional-asymmetry execution: a hard binary rule instead of a continuous throttle.** Mechanism, deliberately simpler than the conservative branch's three-parameter formula: every de-risking order (any cut in `|position|`) fills as an immediate taker, **N=1, always, unconditionally**, no stress proxy consulted; every risk-increasing order rests at a fixed, large patience **N_add ∈ {72,144,216,288}**. One free parameter. Motivated by Perold's implementation-shortfall framing and Almgren & Chriss's (2000, *J. Risk* 3(2); 2001, *J. Portfolio Management*) point that execution urgency should differ by trade *purpose* (a "must" risk-reducing trade vs. an "opportunistic" risk-adding one), not merely scale continuously with a market-stress covariate. Reuses R-77 novel's fill simulator and causal stress/`is_derisking` machinery read-only; the decision rule itself is new. **51 configurations** (45 pre-registered: 6 causality/tamper + 20 core matrix + 4 top-tier + 10 falsification + 4 crash-lag + 1 thesis-reference; + 6 post-hoc diagnostic configs, disclosed, run *after* the core-matrix result to explain a byte-identical plateau, and confirmed not to have affected the already-determined verdict). Pre-registered promotion rule mirrors the conservative branch's (a)-(c)/(e); its own thesis (d): zero or near-zero crash-lag violations even at `N_add` up to 288, because de-risking orders never wait.

**Round total: 100 configurations** (conservative 49 + novel 51).

**Result.** The operator independently reproduced both branches' causality probes and headline crash-lag/thesis numbers from a clean shell before this entry was written — every reproduced figure matched each branch's own report exactly, including a check at a *different* conservative `n_base` (72) than the branch's own headline example (144), which reproduced the identical qualitative pattern (adaptive == matched fixed-N, byte-for-byte). `pytest tests/test_causality_strict.py -q`: 51 passed (both branches' own runs and the operator's independent re-runs). Full suite `python3 -m pytest -q`: 492 passed (both branches' own runs; operator re-run confirms the same).

*Conservative (rescoped adaptive throttle).* No cell clears the ±0.2 Sharpe floor anywhere in validation (best anywhere is inner-train futures entry, Δ+0.074; inner-validation cells run Δ−0.076 to +0.044). Criterion (a) **fails**. (b) ETH falsification **passes** (Δ+0.022 spot, +0.010 futures). (c) BTC pre-2020 control **passes** (Δ+0.057 spot, +0.044 futures). (d) **the thesis test fails decisively, and this is the round's real finding on this branch**: matched to a fee-savings-equivalent fixed N, the adaptive arm's crash-transition-lag violations and mean lag are **byte-identical** to the fixed-N arm at the same effective N (operator-reproduced at both `n_base=72` and the branch's own `n_base=144`) — the stress override fires only twice across all 124 flip-to-flat events, both at the one genuinely sharp crash in the window (2020-03-25, COVID), and never for the slow-burn 2019-01-20 near-miss (the same bar R-56 and R-77 both already flagged) that now resolves as a 144-bar, 12-hour taker fallback at `n_base=144`. Outside a sharp, already-obvious spike, the "adaptive" mechanism is functionally the fixed-N mechanism it was built to improve on. (e) plateau **passes**, but it is a plateau of *no material effect from n_base or κ*, not of good performance (inner-train Sharpe spread 1.887–1.890 across all 8 cells). Per the AND-rule, (a) and (d) both failing is more than sufficient: **NEGATIVE**.

*Novel (directional asymmetry).* Core matrix (entry tier, all four `N_add` byte-identical within every cell): best delta +0.055 (futures/inner-train), worst −0.060 (futures/inner-val); nowhere near ±0.2 in either direction. Fee savings are real and comparable to R-56/R-77's own magnitude for similar N (9–11% entry tier, 46–48% top tier). Criterion (a) **fails decisively**. (b) ETH falsification **passes** (Δ+0.016 to +0.019). (c) BTC pre-2020 control **passes** (Δ+0.030 to +0.032). (d) **the thesis test passes decisively, and this is the round's real finding**: **0/124 crash-transition-lag violations, mean lag pinned at the 1-bar floor, at every N_add from 72 to 288** — genuinely better than R-77's own N≤24 baseline (5/124, mean 1.31), operator-reproduced bit-for-bit via the branch's own `crashlag` command. Falsification test 5 (would the danger persist anyway despite guaranteed-immediate de-risking?) did not trigger — the round's diagnosis that R-56's danger was specifically about de-risking-order patience holds up. (e) plateau **passes**, but a disclosed post-hoc diagnostic (run after the core-matrix result, exploratory, did not change the already-determined verdict) found it is partly a ceiling effect: no risk-increasing order in the core matrix ever waited past 71 bars on this dataset, so N_add=144/216/288 are functionally inert here — the mechanism was shown to cost nothing at that scale, not that it was needed at it. Per the AND-rule, (a) failing alone is sufficient: **NEGATIVE**.

**Verdict.** **NEGATIVE, both branches.** One-line lesson: R-77's own named open question is now closed with a clean, asymmetric answer rather than an untested scope gap — a hard, direction-based execution rule *can* eliminate R-56's crash-transition-lag danger completely, even 12x past the N≥72 threshold that caused it (novel branch: 0/124 violations vs. R-77's own 5/124 baseline), while a smoothly-scaled continuous throttle tuned at small `n_base` does *not* automatically inherit that property when its ceiling is simply raised (conservative branch: byte-identical to naive fixed-N at every `n_base` tested) — the earlier lesson that a smarter-looking formula is not automatically a safer one generalizes cleanly from R-77's N≤24 scope to the actual danger zone. Neither result moves the strategy's own Sharpe or drawdown, which makes this the fourth consecutive execution-model round (R-56, B-24/R-77-conservative, R-77-novel, now both R-78 branches) to find that real, substantial fee savings (9–23% depending on tier and branch) at this strategy's own turnover and return variance are simply too small relative to the ±0.2 noise floor to move a promotion decision — independent of how well-designed the execution mechanism is on its own terms, and independent of whether it also happens to solve a genuine risk problem cleanly. Neither decision rule moved after looking: both branches' promotion criteria and falsification tests were frozen before any run and applied exactly as written; the novel branch's six post-hoc diagnostic configs were explanatory (why the plateau is flat), not a re-opening of an already-determined verdict, and are disclosed rather than folded silently into the pre-registered count.

**Holdout counter.** **+0.** Both branches' working frames are cut to ≤2022-12-31 at load time with a runtime `assert_no_holdout()` guard; the operator independently grepped both new files for any literal `202[3-9]` date outside prose citations and confirmed none is used in any load/slice call, and independently re-ran both branches' causality probes and headline crash-lag numbers from a clean shell (above). Running program-level total stays **~637** (R-72/R-73/R-74/R-75/R-76/R-77's figure, unchanged) — see the bullet added below in [Holdout consultations to date](#holdout-consultations-to-date).

**Next step.** This closes the execution-model line on `kelly_regime_v4` more thoroughly than R-77 did: the one specific scope gap R-77 named (adaptivity untested at N≥72) has now been tested by two structurally different constructions, and the honest conclusion is that the crash-lag danger R-56 found is easy to eliminate completely (the novel branch's binary directional rule) but doing so still does not buy a promotable Sharpe or drawdown improvement — so a fifth execution-model round on this strategy, of any construction, is not recommended without a specific new reason to expect the needle to move on the Sharpe/DD axis rather than the crash-lag axis, which is now closed. The novel branch's directional-asymmetry primitive is simple (one parameter), cheap (real fee savings, no downside found anywhere in this round's validation matrix), and cleanly eliminates a previously-documented, real risk property (R-56's crash-transition-lag danger) independent of any Sharpe case for it — filed as **B-38** (LOW priority, a risk-hygiene candidate rather than a return claim: "adopt guaranteed-immediate-fill on de-risking orders as kelly_regime_v4's live/paper-trading execution default, independent of any Sharpe promotion, since this round found it costs nothing detectable and removes a documented danger") rather than promoted now, since building or wiring a live limit-order execution path purely to bank a risk property nobody has asked for yet, with no registered strategy currently using anything but immediate fills in production, is exactly the speculative-infrastructure ROUTINE.md warns against building ahead of need. **B-06 (forward paper trading, ongoing since R-48, advancing on its own via the scheduled workflow) remains the highest-value item on merit and the standing zero-cost recommendation.**
...
| what | why | ref |
|---|---|---|
| Regime-adaptive execution urgency on `kelly_regime_v4` (R-77's own continuous formula, unchanged), rescoped so `n_base ∈ {72,144,216,288}` genuinely spans R-56's N≥72 crash-transition-lag danger zone | 49 configurations. No cell clears the ±0.2 Sharpe floor anywhere in validation (best +0.074, train only). The round's actual thesis fails decisively: matched to a fee-savings-equivalent fixed N, the adaptive arm's crash-transition-lag violations and mean lag are byte-identical to plain fixed-N at the same effective N — the stress override fires only twice across 124 events, both at the one genuinely sharp crash (2020-03-25 COVID), never for the slow-burn 2019-01-20 near-miss that dominates the lag statistic (now a 144-bar taker fallback at `n_base=144`). Do not re-try this continuous kappa-scaled adaptive-urgency formula expecting it to inherit crash-lag safety merely by raising its `n_base` ceiling — the mechanism is functionally the fixed-N model it was meant to improve on outside a sharp, already-obvious stress spike. | R-78 (conservative) |
| Directional-asymmetry execution on `kelly_regime_v4` — every de-risking order fills as an immediate taker (N=1, unconditional), every risk-increasing order rests at a fixed patient `N_add ∈ {72,144,216,288}` (Perold implementation-shortfall; Almgren & Chriss 2000/2001) | 51 configurations (45 pre-registered + 6 disclosed post-hoc diagnostics that did not affect the verdict). Cleanly eliminates R-56's crash-transition-lag danger (0/124 violations, mean lag pinned at the 1-bar floor, at every `N_add` up to 288 — better than R-77's own N≤24 baseline's 5/124) with real fee savings (9–11% entry tier) and no downside found on ETH or BTC pre-2020 falsification — but never moves inner-validation Sharpe beyond ±0.06 in either direction, nowhere near the ±0.2 promotion floor. Do not re-try expecting a Sharpe/drawdown promotion from this mechanism alone — the crash-lag danger axis is now closed (this construction solves it completely), but that property does not by itself buy a Sharpe or drawdown win at this strategy's turnover and return variance; see B-38 for the standing, un-promoted risk-hygiene note this round filed instead. | R-78 (novel) |
...
**Re-ranked 08-21 after R-78.** R-77's own "next step" named one specific, non-duplicate open question rather than a filed backlog item: whether adaptive execution urgency helps specifically in the N≥72 region R-56 found genuinely dangerous. This round worked that question directly with two structurally different constructions. Conservative branch: R-77's own continuous adaptive-urgency formula, unchanged, rescoped so `n_base` genuinely spans 72-288. Result: it does **not** inherit crash-lag safety merely from a raised ceiling — matched to a fee-savings-equivalent fixed N, its crash-transition-lag violations and mean lag are byte-identical to plain fixed-N, because the stress override only ever fires on an already-obvious sharp crash (one event, COVID) and never on the slow-burn near-miss that dominates the lag statistic. Novel branch: a hard directional-asymmetry rule (de-risking orders always fill immediately, N=1; risk-adding orders stay patient up to N_add=288) — genuinely different in kind, one free parameter instead of three. Result: it **does** eliminate the crash-lag danger completely (0/124 violations at every N_add tested, versus R-77's own N≤24 baseline of 5/124) — the cleanest execution-safety result this axis has produced — but like every execution-model round before it, the real fee savings this buys (9-11% at the entry tier) never translate into a Sharpe or drawdown improvement past the ±0.2 noise floor. Both **NEGATIVE** on the promotion bar. This closes the execution-model line on `kelly_regime_v4` more thoroughly than R-77 did: the crash-lag-danger question is now answered (a simple binary rule solves it; a smarter-looking continuous formula does not, automatically), and the Sharpe/drawdown question has now failed identically across four independent constructions (R-56, R-77 conservative, R-77 novel, both R-78 branches) — a fifth execution-model round of any construction on this strategy is not recommended without a specific new reason to expect the Sharpe/DD needle to move. The novel branch's directional-asymmetry primitive is filed as **B-38** (LOW priority, a risk-hygiene note rather than a return claim — see its own row) rather than promoted or wired into any live path, since nothing currently in production uses anything but immediate fills and building a limit-order execution path purely to bank an unrequested risk property would be exactly the speculative infrastructure `docs/ROUTINE.md` warns against. **Nothing is left genuinely OPEN on the backlog.** `scripts/paper_trade.py` (B-06, ongoing since R-48, advancing on its own via the scheduled GitHub Actions workflow) is, once again, the only standing recommendation that costs nothing and needs no new idea.
...
| **B-38** | Adopt R-78 novel's directional-asymmetry execution rule (every de-risking order fills as an immediate taker, unconditionally; risk-adding orders stay patient) as `kelly_regime_v4`'s live/paper-trading execution default, independent of any Sharpe promotion — this round found it costs nothing detectable across the full validation matrix (no cell moved against it) and cleanly removes a documented, real risk property (R-56's crash-transition-lag danger, 0/124 violations vs. R-77's 5/124 fixed-N baseline) | COST (risk hygiene, not a return claim) | LOW | Filed by R-78. Not a promotion candidate — it never cleared the Sharpe/drawdown bar, so per `docs/ROUTINE.md` nothing is registered or wired into any live path. Named because it is the first execution-model construction in four rounds (R-56, R-77 ×2, R-78 conservative) to actually solve the crash-lag danger rather than merely avoid or inherit it, at zero measured cost — but adopting it live would require building a limit-order execution path `scripts/paper_trade.py`/`live_bot.py` do not currently have (both use immediate fills only), which is speculative infrastructure ahead of any strategy that would need it. Revisit only if a future round's live-execution work is undertaken for an unrelated reason and this can be folded in cheaply. |
...
- **08-21 · ~637** — R-78: **+0** on top of R-77's ~637 (unchanged). Both
  branches' working frames are cut to ≤2022-12-31 at load time with a
  runtime `assert_no_holdout()` guard; the operator independently grepped
  both new files for any literal `202[3-9]` date outside prose citations
  and confirmed none is used in any load/slice call, and independently
  reproduced both branches' causality probes and headline crash-lag/thesis
  numbers from a clean shell, including at a conservative-branch `n_base`
  the branch's own report did not headline (72, not just 144) — both
  matched exactly.
