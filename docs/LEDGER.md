...
| **B-22** | A magnitude-*and*-duration filter (or a confirming, non-overriding combination rule) on the aggregate-USDT-stablecoin-supply-deceleration signal R-54's novel branch built — the signal itself was confirmed to LEAD `kelly_regime_v4`'s price-anchor gate (9/12 episodes, median +16.5 days), the first INFO-axis signal in this project's three attempts (B-07/R-44 on-chain, R-53 macro, now this) to do so, but a single-threshold hard veto fed by it fires on transient supply noise as often as on genuine stress and loses the money it saves | INFO | OPEN | R-54's own diagnosis: the threshold tight enough to catch real stress early (thresh=0.75, 24 stress-onsets) also fires on noise; loosening it (thresh=1.25) cuts false alarms but gives back most of the lead time, converging back toward v4 rather than improving on it — timing and precision are separate axes and this signal has only solved the first one. Two concrete next steps named by R-54's own report: (1) require the vote to persist for a minimum number of days before it can veto, rather than firing on any single-bar threshold crossing; (2) feed the signal into R-53's originally pre-registered precision-weighted-average architecture (a confirming speed-up, not a unilateral override) instead of the hard-veto one, which R-54 deliberately did not re-test — it might behave differently now that the feeding signal actually leads instead of lags. |
