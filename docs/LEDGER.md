...
| **B-18** | Is the `kelly_regime_covkelly` allocator's monthly/weekly cadence-inconsistency (R-42, attenuated but not resolved by R-43's mean-denoising) actually a rebalance-engine/segment-restart artifact rather than a mean-estimation-noise problem? | ERR | OPEN | Named as an open question by R-43's novel-branch author: since de-noising the mean shrank but did not eliminate the monthly-loses/weekly-wins flip, the remaining defect may be structural (each rebalance segment's `kelly_regime_v4` sub-book restarts flat/unlatched rather than carrying position forward — a documented simplification in both `kelly_regime_covkelly.py` and `_v2.py`) rather than about `mu`/`Sigma` at all. Worth checking before spending more effort on estimator variants. R-49's native-engine design note independently names this same segment-restart limitation as a plausible root cause and the kind of problem that design would remove — the two backlog items may resolve together. Now the highest-priority genuinely OPEN item, B-06/B-08/B-17 having each moved to DONE/ONGOING/PARTIAL. |
