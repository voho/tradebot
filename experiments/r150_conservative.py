"""R-150 CONSERVATIVE branch: bolt kelly_regime_v3/v4's own (unretuned)
conditional-volatility-target scale onto champions_council, replacing its
plain, continuous ``min(target_vol/v, max_leverage)`` portfolio-level scale.

Executes the *conservative* half of the frozen pre-registration in
``experiments/r150_shared.py`` (read that file's module docstring for the
...
- mechanism: ``conservative_target`` (imported, not reimplemented) --
  champions_council's own Hedge-blended vote, byte-identical to the control,
  scaled by v4's own conditional-volatility-target factor instead of the
  plain continuous ``min(target_vol/v, max_leverage)`` ratio.
- Step-0 A2 kill switch: R^2 of ``conservative_target`` vs
  ``champions_council_target`` on inner-train. If > 0.98, STOP.
...
- B6 (mandatory whenever B1 passes on any market, per R-132/R-135's own
  lesson): a zero-information, turnover-matched control
  (``champions_council_target_deadband``) must NOT itself clear B1's own
  bar against the true control. If it does, the candidate's B1 pass is a
  turnover-reduction artifact and the verdict is NEGATIVE regardless of
  A2-B5.
- pytest: causality-strict + full suite, to confirm nothing outside this
  file broke (this branch touches no file except this one).

This file does not modify ``r150_shared.py`` and does not commit anything,
...
def run_a2() -> float:
    ...
    ctrl = champions_council_target(inner_train)
...
if __name__ == "__main__":
    print("=" * 100)
    print("R-150 CONSERVATIVE branch: conditional-vol-target scale on champions_council")
    print("=" * 100)

    # ---- A2 kill switch ----
    print("\n--- Step-0 A2 kill switch (R^2 vs champions_council_target, inner-train) ---")
    r2 = run_a2()
    print(f"R^2(conservative_target, champions_council_target) on inner-train = {r2:.6f}")
    if r2 > 0.98:
    ...
    # primary market for B3/B6: whichever market B1 passed on; SPOT if both/neither
    ...
    print(f"\nB1 primary market for B3/B6: {primary_market_name} "
    ...
    # ---- B6: mandatory zero-information turnover-matched control, whenever B1 passes ----
    print("\n--- B6: zero-information turnover-matched control (mandatory since B1 passed) ---")
    if b1_pass:
        cand_inner_val_trades = next(
            (r["cand_trades"] for r in inner_val if r["market"] == primary_market_name), None)
        if cand_inner_val_trades is None:
            raise AssertionError(
                f"B6: could not find cand_trades for primary_market={primary_market_name} "
                f"in inner_val rows")
        b6 = run_b6(primary_market=primary_market, cand_inner_val_trades=cand_inner_val_trades)
        print(f"trials_trade_counts (deadband -> inner_val trades): {b6['trials_trade_counts']}")
        print(f"chosen_deadband={b6['chosen_deadband']:.2f}  "
              f"chosen_trades={b6['chosen_trades']}  cand_trades={b6['cand_trades']}")
        print(f"d_sharpe (deadband-only control vs true control) = {b6['d_sharpe']:+.4f}   "
              f"boot=[{b6['boot_lo']:+.4f}, {b6['boot_hi']:+.4f}]")
        b6_artifact_detected = b6["artifact_detected"]
        print(f"artifact_detected = {b6_artifact_detected}")
    else:
        b6 = None
        b6_artifact_detected = False
        print("B1 did not pass on any market -- B6 is skipped (nothing to reclassify as an "
              "artifact; there is no B1 pass to protect against being one).")

    b6_pass = not b6_artifact_detected
    ...
    if b6 is not None:
        print(f"B6 (turnover-matched control, mandatory since B1 passed): "
              f"chosen_deadband={b6['chosen_deadband']:.2f}  artifact_detected={b6_artifact_detected}  "
              f"-> pass={b6_pass}")
    else:
        print("B6 (mandatory only if B1 passes): skipped, B1 did not pass on any market -> pass=True (moot)")
    print(f"pytest causality-strict: returncode={rc1}")
    print(f"pytest full suite:       returncode={rc2}")

    verdict = "PROMOTE-CANDIDATE" if (
        r2 <= 0.98 and b1_pass and b4_pass and b5_pass and b6_pass
    ) else "NEGATIVE"
    print(f"\nVERDICT: {verdict}")
