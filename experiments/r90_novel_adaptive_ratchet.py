...
def main() -> None:
    ...
    # --- supplementary, NOT part of the frozen decision rule: same stop
    # distance (finalist's k), INSTANT unconditional restart (the naive
    # construction this branch's restart gate is meant to improve on), so
    # the whipsaw rate above has something concrete to be compared against.
    hdr("SUPPLEMENTARY (exploratory, not part of the frozen decision rule): "
        "same k, INSTANT restart")
    print("Same ATR-scaled stop distance as the finalist (k="
          f"{finalist.k}), but reentry_reclaim=False and "
          "reentry_delay_bars=None -- the naive")
    print("unconditional-instant-resume construction Hsieh's paper motivates a "
          "restart mechanism to fix,")
    print("run on the IDENTICAL stop distance so the only thing that differs "
          "is the restart gate.\n")
    naive_key = ("naive_instant", finalist.k) + _key(train)
    if naive_key not in _CACHE:
        raw_tr = cached_raw(train)
        atr_tr = atr_days(train, ATR_DAYS)
        close_tr = train["close"].to_numpy(dtype=float)
        sf_tr = np.where(np.isfinite(atr_tr),
                         np.minimum(0.95, finalist.k * atr_tr / close_tr), 1.0)
        _CACHE[naive_key] = apply_trailing_stop(train, raw_tr, sf_tr,
                                                reentry_delay_bars=None,
                                                reentry_reclaim=False)
    naive_run = _CACHE[naive_key]
    naive_target_train = apply_deadband(naive_run.target)
    naive_whip = stopout_whipsaw_rate(close_train, naive_target_train,
                                      naive_run.stop_events)
    print(f"naive instant restart : stop_events={naive_whip['stop_events']}  "
          f"events_with_reentry_in_horizon="
          f"{naive_whip['events_with_reentry_in_horizon']}  "
          f"whipsaws={naive_whip['whipsaws']}  "
          f"whipsaw_rate={naive_whip['whipsaw_rate']:.4f}  "
          f"mean_log_cost={naive_whip['mean_whipsaw_log_cost']:+.5f}")
    print(f"reclaim+cooldown gate : stop_events={whip['stop_events']}  "
          f"events_with_reentry_in_horizon="
          f"{whip['events_with_reentry_in_horizon']}  "
          f"whipsaws={whip['whipsaws']}  "
          f"whipsaw_rate={whip['whipsaw_rate']:.4f}  "
          f"mean_log_cost={whip['mean_whipsaw_log_cost']:+.5f}")
    print("\nNOTE, worth a skeptical reader's attention: the reclaim gate "
          "REQUIRES close[i] > exit_price")
    print("before re-arming at all, which is EXACTLY this diagnostic's own "
          "definition of a whipsaw")
    print("(re-entry at a close higher than the exit). So conditional on a "
          "re-entry happening inside")
    print("the horizon, the reclaim gate cannot mechanically LOWER the "
          "whipsaw-rate statistic as defined")
    print("here -- if anything it raises the floor, since arming itself "
          "already requires the price to")
    print("sit above the exit level. What the gate COULD still do, and what "
          "this diagnostic does not")
    print("separately measure, is reduce the FREQUENCY of stop-and-reentry "
          "cycles (fewer stop_events")
    print("under cooldown: 75 with cd=3d vs "
          f"{naive_whip['stop_events']} with instant restart on the same k) "
          "and delay each re-entry to a")
    print("point further from the exit rather than the very next bar the "
          "vote wants long -- which is why")
    print("mean_log_cost per whipsaw is reported alongside the rate, not "
