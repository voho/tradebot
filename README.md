...
Earlier capital-scale checks found proportional results apart from exchange
minimum-order effects. The R-189 candidates were evaluated at $1,000; test
other balances with
...
Earlier rounds found that sizing and turnover control mattered more than
adding directional indicators. R-189 adds ten game-theory candidates; none
clears its frozen promotion bar. Two achieve several fills per day, but
none achieves several completed round trips per day.
...
ranks by, **not one of the 36 alternatives is distinguishably better than
...
> is**: measured on the 25 strategies registered then, the gap is *not* a
...
> that measurement concerned the other 17 strategies then registered.
> The ten R-189 additions also use equity-notional sizing and inherit
> the coarser futures rebalance band.
...
> the original R-29 audit found **10 of 96** adjacent differences with a
...
> P = 0.52**. In that historical audit, no strategy cleared its deflated-Sharpe
> bar against the then-counted 190 trials. R-189 separately accounts for
> its larger consultation count; those old ordering counts are not a
> fresh test of the expanded 37-strategy roster.
...
> and no current alternative has a positive lower growth bound. Detail in
...
2023–2026 beta test uses fees, slippage, ETH replication, matched perpetual
funding and 24 paired windows per market. **None is promoted.**

| candidate meeting 2–6 fills/day | spot balance from $1,000 | fills/day | completed round trips/day |
|---|---|---|---|
| `qre_council` | $1,289 | 3.04 | 0.000 |
| `nash_council` | $875 | 2.37 | 0.187 |

Same-period spot holding finishes $3,839; Kelly v4 finishes $3,392.
QRE's fills are rebalances of a continuously long position. Neither these
two nor the other eight provides a few completed trades/day with a validated
advantage. See [all results and reproduction commands](reports/r189_games/README.md)
and [the primary-source research](docs/R189_RESEARCH.md).

![R-189 candidates, results and actual cadence](reports/r189_games/candidates.png)

_The futures pane is a 5x market. Candidates target at most 1x; the 5x
buy-and-hold stress control is liquidated, so its zero tail disappears on
the log axis. Funding uses matching Deribit data. The main comparison table
above retains its historical zero-slippage, unfunded-proxy convention._
...
**Earlier roster balance curves** (spot, $1,000 start; R-189 is charted above; the palette is
